...
func _ready():
	...
	for i in range (house_spawn_points.size()):
		_spawn_house(house_mocks.houses[i].texture, house_spawn_points[i].global_position,house_mocks.houses[i].user_name )
	
func _spawn_house(texture: Texture2D, position: Vector2, user_name: String):
